...
#
# # TODO: these notes are very ass.. need to remake later
#
#
...
# NOTE: the above notes are pretty disorganized.. will fix it up later. for now
# i make some new notes below that are in my regular style of separating
# sections by functions.
...
file_test_operators() {
    # file test operations , also known as file test expressions are arguments
    # to the `test` command. you will often see them in if statements like so:

    if [[ -e "notes.sh" ]]; then
	echo "notes.sh exists"
    fi
}
